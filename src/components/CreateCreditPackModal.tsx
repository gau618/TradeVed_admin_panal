...
export default function CreateCreditPackModal({ onCreated, onClose }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      await api.post('/admin/creditpacks', {
        name: form.name,
      ...
      setError(err.response?.data?.message || 'Failed to create credit pack');
    } finally {
      setLoading(false);
    }
  }
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-2xl w-full max-w-md border border-gray-200">
        <div className="p-6 border-b border-gray-200">
          <div className="flex items-center">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-900 mb-1">Create Credit Pack</h2>
              <p className="text-sm text-gray-600">Add a new credit package</p>
            </div>
          </div>
        </div>
